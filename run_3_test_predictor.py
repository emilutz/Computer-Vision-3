...
if __name__ == '__main__':
	...
	for image_index in range(TEST_SIZE):
		try:
			...
			crops = crop_5(img, IMG_SIZE).reshape((5, IMG_SIZE, IMG_SIZE, 1))

			feed_dict = {
							is_training : False,
							angle : np.asarray([0]),
							images_tst : crops,
							labels_tst : np.zeros((5, 1))
						}
			...
			plt.bar(list(range(CLASSES)), np.sum(probs, axis=0), width=0.5)
			plt.xticks(list(range(CLASSES)), list(class_to_label.keys()), rotation='vertical')
			plt.show()
